...
class SignUpView(View):
    ...
    def post(self, *args, **kwargs):
        ...
        return JsonResponse({"data":{"self.request": self.request.POST}})
...
class LoginView(View):
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if user.id:
                self.request.session['_auth_user_id'] = _meta.pk.value_to_string(user.id)
                response_data = {"data":{"self.request.POST": self.request.POST, "self.request.session['_auth_user_id']": self.request.session['_auth_user_id'],"username": username}}
...
#     item = get_object_or_404(Item, slug=request.slug)nn
#     review = Review(request.title, request.body, item)
#     review.save()
#     return redirect("/")
...
#     item = get_object_or_403(Item, slug)
