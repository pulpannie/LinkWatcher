...
from .forms import BountyForm, BanUser_Form
...
from .forms import ProtectForm, EditAnswerForm, NewUserForm, CommentForm
...
class SignUpView(View):
    ...
    def post(self, *args, **kwargs):
        ...
        print("could not sign up", form.errors)
        return redirect('registration/signup.html')
    
class new_question(View):
    def get(self, *args, **kwargs):
        form = QuestionForm()
        context = {'form': form}
        return render(self.request, 'qa/new_question.html', context)
    def post(self, *args, **kwargs):
        AllTags = Tag.objects.all().values_list('name', flat=True)
        form = QuestionForm(self.request.POST, self.request.FILES)
        if form.is_valid():
            formTags = form.cleaned_data.get('tags')
            gettingBody = form.cleaned_data.get('body')
            gettingTitle = form.cleaned_data.get('title')
            username = form.cleaned_data.get('post_owner')
            user = User.objects.get(username=username)
            new_post = Question(tags=formTags, body=gettingBody, title=gettingTitle, post_owner = user.username)
            new_post.save()
            ...
            return redirect('qa:new_question')
        context = {'form': form}
        return render(self.request, 'qa/new_question.html', context)
    
class questionDetailView(View):  # slug):
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            gettingBody = form.cleaned_data.get("body")
            gettingWiki = False
            gettingWiki = bool(gettingWiki)
            new_post = Answer(body=gettingBody, is_wiki_answer=gettingWiki, answer_owner=user, username=user.username, questionans=data, tmp=data.pk)
        ...
        # Revival Tag - START
            ...
            post_owner = User.objects.get(username=data.post_owner)
        ...
        # WORKED
            messages.success(self.request, "Succeeded in creating answer")
            return redirect(
                '/',
                pk=data.id,
            )  # slug=slug)
...
class save_comment(View):
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            user = User.objects.get(username=username)
            que_user = User.objects.get(username=que.post_owner)
            if comment != "":
                createdComment = CommentQ.objects.create(
                    question_comment=que, comment=comment, commented_by=user, username=user.username, c_id = que.pk)
                if self.request.user != que.post_owner:
                    Notification.objects.create(
                        noti_receiver=que_user,
                        noti_receiver_username = que.post_owner,
            ...
            # print error of User.DoesNotExist.
                messages.success(self.request, "Successfully created comment")
                return redirect('qa:new_question')
