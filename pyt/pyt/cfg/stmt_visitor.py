...
class StmtVisitor(ast.NodeVisitor):
    ...
    def add_to_definitions(self, node):
        ...
        local_qualified_name = '.'.join(local_definitions.classes +
                                        [node.name])
        node.name = local_qualified_name
    ...
    def handle_or_else(self, orelse, test):
        ...
        if isinstance(orelse[0], ast.If):
            ...
            return else_connect_statements
...
#        print("self.nodes: ", self.nodes)
        ...
        # print("node.test", node.test, type(node.test))
        #link-TEE
        #print("first_statement: ", body_connect_stmts)
        body_node = body_connect_stmts.first_statement

        while(body_node not in body_connect_stmts.last_statements and test not in body_node.if_statements_seen and body_node != []):
            body_node.if_statements_seen.append(test)
            #print("body_node: ", body_node, "\nbody_node outgoing: ", body_node.outgoing[0], "\nlast statements: ", body_connect_stmts.last_statements, "\n")
            body_node = body_node.outgoing[0]
        ...
        if node.orelse:
            #for o in node.orelse:
                #print("orelse: ", o)
            orelse_nodes = self.handle_or_else(node.orelse, test)
            if (not isinstance(orelse_nodes, list)):
                orelse_last_nodes = orelse_nodes.last_statements
                body_connect_stmts.last_statements.extend(orelse_last_nodes)

            #link-TEE
            #print("orelse: ", orelse_nodes)
                orelse_node = orelse_nodes.first_statement
                while(orelse_node not in orelse_nodes.last_statements and test not in orelse_node.if_statements_seen and orelse_node != []):
                    orelse_node.else_statements_seen.append(test)
                    #print("body_node: ", body_node, "\nbody_node outgoing: ", body_node.outgoing[0], "\nlast statements: ", body_connect_stmts.last_statements, "\n")
                    orelse_node = orelse_node.outgoing[0]
    ...
    def visit_Try(self, node):
        ...
        if node.orelse:
            orelse_nodes = self.handle_or_else(node.orelse, body.last_statements[-1])
            orelse_last_nodes = orelse_nodes.last_statements
    ...
    def visit_Assign(self, node):
        if(node.value.__class__.__name__ == "BinOp"):
            #print("visit_assign function, node value: ", node.value.left.attr, node.value.op, node.value.right, node.value.__class__.__name__)
            rhs_visitor = VarsVisitor()
            rhs_visitor.visit(node.value)
        else:
            rhs_visitor = RHSVisitor()
            rhs_visitor.visit(node.value)
        #print("visit_assign function, rhs_visitor: ", rhs_visitor, rhs_visitor.result)
