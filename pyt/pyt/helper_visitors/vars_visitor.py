...
class VarsVisitor(ast.NodeVisitor):
    ...
    def visit_Attribute(self, node):
        if self.object_repr == "":
            self.object_repr = node.attr
        else:
            self.object_repr = node.attr + "." + self.object_repr
        if node.value.__class__.__name__ == "Name":
            self.object_repr = node.value.id + "." + self.object_repr
        if not isinstance(node.value, ast.Name):
            self.visit(node.value)
        else:
            #self.result.append(node.value.id + "." + node.attr)
            #self.object_repr = node.attr + "." + self.object_repr
            self.result.append(self.object_repr)
