...
from ..core.node_types import AssignmentNode, IfNode, EntryOrExitNode
...
class ReachingDefinitionsTaintAnalysis():
    ...
    def fixpointmethod(self, cfg_node):
        ...
        # Assignment check
        #if isinstance(cfg_node, IfNode):
        #print(cfg_node, cfg_node.ast_node)
    ...
    def arrow(self, JOIN, _id):
        ...
        for node in self.lattice.get_elements(JOIN):
            ...
            #edited for link-TEE
            is_lhs_attribute = len(node.left_hand_side.split(".")) > 1
            if is_lhs_attribute:
                if _id in node.left_hand_side:
                    r = r ^ self.lattice.el2bv[node]
            elif node.left_hand_side == _id:
                r = r ^ self.lattice.el2bv[node]
